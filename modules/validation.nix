...
let
  ...
  validHostFields = [ "users" "sing-box" "wireguard" ];
  validUserFields = [ "git" "ssh" ];
  validGitFields = [ "name" "email" "signingKey" ];
  validSshFields = [ "authorizedKeys" "hosts" ];
  validSshHostFields = [ "hostname" "user" "port" "identityFile" "sopsSecret" "forwardX11" "extraOptions" ];
  validSingBoxFields = [ "enable" "settings" ];
  validWireguardFields = [ "enable" "interfaces" ];

  # Check function: returns null if valid, error message if invalid
  checkFields = validFields: context: attrs:
    let
      fields = builtins.attrNames attrs;
      unknown = lib.filter (f: !builtins.elem f validFields) fields;
    in
    if unknown == [] then null
    else "Unknown fields in ${context}: ${lib.concatStringsSep ", " unknown}";

  # Validate SSH hosts entries
  validateSshHosts = hostContext: hosts:
  ...
  validateUser = hostName: userName: user:
    let
      ...
      sshHostErrors = if (user.ssh.hosts or {}) != {}
        then validateSshHosts context user.ssh.hosts
        else [];
    in
    errors ++ sshHostErrors;

  # Validate host profile
  validateHost = hostName: hostProfile:
    let
      ...
      userErrors = lib.concatMap (userName:
        validateUser hostName userName ((hostProfile.users or {}).${userName} or {})
      ) (builtins.attrNames (hostProfile.users or {}));
    in
    hostErrors ++ userErrors;

  # Validate all hosts
  allErrors = if profiles == {} then []
    else lib.concatMap (hostName:
      validateHost hostName (profiles.${hostName} or {})
