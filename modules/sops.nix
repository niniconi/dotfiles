...
{ config, lib, pkgs, hostName, users, profiles, ... }:

let
  hasSecrets = builtins.pathExists ../secrets/default.nix;

  # Generate SSH secrets for each user
  allUserSecrets = lib.foldl' (acc: userName:
    let
      hostProfiles = profiles.${hostName} or {};
      profile = hostProfiles.users.${userName} or {};
      ssh = profile.ssh or {};
      hosts = ssh.hosts or {};
      validHosts = lib.filter (h: h.sopsSecret or null != null)
        (builtins.attrValues hosts);

      userSecrets = lib.listToAttrs (lib.concatMap (host:
        let
