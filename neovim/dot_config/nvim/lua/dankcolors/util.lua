...
function M.invert(color)
  ...
  elseif type(color) == "string" then
    local hsluv = require("dankcolors.hsluv")
    if color ~= "NONE" then
...
---@param groups dankcolors.Highlights
...
function M.cache.file(key)
  return vim.fn.stdpath("cache") .. "/dankcolors-" .. key .. ".json"
end

---@param key string
function M.cache.read(key)
  ---@type boolean, dankcolors.Cache
...
---@param key string
---@param data dankcolors.Cache
function M.cache.write(key, data)
  pcall(M.write, M.cache.file(key), vim.json.encode(data))
end

function M.cache.clear()
  uv.fs_unlink(M.cache.file("dark"))
  uv.fs_unlink(M.cache.file("day"))
end
